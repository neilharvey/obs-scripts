-- Source: https://obsproject.com/wiki/Scripting-Tutorial-Source-Shake
obs = obslua
...
is_active = false -- Global animation activity flag
...
Shake a source in the current scene when a hotkey is pressed.  
    
Go to settings then Hotkeys to select the key combination.  
...
function script_properties()
    ...
    local list_property = obs.obs_properties_add_list(props, "source_name", "Source name", obs.OBS_COMBO_TYPE_LIST,
        obs.OBS_COMBO_FORMAT_STRING)
    populate_list_property_with_source_names(list_property)
    -- Button to refresh the drop-down list
    obs.obs_properties_add_button(props, "button", "Refresh list of sources", function()
        populate_list_property_with_source_names(list_property)
        return true
    end)
