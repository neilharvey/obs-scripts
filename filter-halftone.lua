-- Source: https://obsproject.com/wiki/Scripting-Tutorial-Halftone-Filter
...
function script_description()
    ...
    This Lua script adds a video filter named Halftone.     
    The filter can be added to a video source to reduce the number of colors of the input picture. 
    It reproduces the style of a magnified printed picture.]]
end
...
source_info.output_flags = obs.OBS_SOURCE_VIDEO -- Combination of VIDEO/AUDI/ASYNC/etc
...
source_info.get_properties = function(data)
    ...
    obs.obs_properties_add_button(props, "pattern_reset", "Reset pattern", function()
        obs.obs_data_set_string(data.settings, "pattern_path", "");
        data.pattern = nil;
        return true;
    end)
    ...
    obs.obs_properties_add_button(props, "palette_reset", "Reset palette", function()
        obs.obs_data_set_string(data.settings, "palette_path", "");
        data.palette = nil;
        return true;
    end)
...
-- Destroys and release resource linked to the custom data
